...
from api.dependencies import get_image_processor, get_inference_service
...
async def detect(
    ...
    inference_service: InferenceServiceBase = Depends(get_inference_service),
    image_processor: ImageProcessorBase = Depends(get_image_processor),
):
    """
    ...
    try:
        ...
        logger.info(
            f"Processing image: {file.filename}, size: {len(image_bytes)} bytes"
        )
        ...
        logger.info(
            f"Detected {len(detections)} objects with conf_threshold={conf_threshold}"
        )
